...
let htmlmin = require("gulp-htmlmin"); // под вопросом !!!
...
function fontstyle() {
	let file_content = fs.readFileSync(src_folder + '/scss/fonts/fonts.scss');
	if (file_content == '') {
		fs.writeFile(src_folder + '/scss/fonts/fonts.scss', '', cb);
			...
			if (items) {
				...
				for (var i = 0; i < items.length; i++) {
					...
					if (c_fontname != fontname) {
						fs.appendFile(src_folder + '/scss/fonts/fonts.scss', '@include font("' + fontname + '", "' + fontname + '", "400", "normal");\r\n', cb);
					}
					c_fontname = fontname;
				}
			}
